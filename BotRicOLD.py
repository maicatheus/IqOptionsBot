...
def RodarSinais(count,hora,minuto,segundo,timeframe,acao,martingale,entrada,delay,asset):
	count = int(count)
	hora= int(hora)
...
API.connect()
API.change_balance(configuracao('tipo_conta')) # PRACTICE / REAL
...
for sinal in lista:
	...
	thread = threading.Thread(target = RodarSinais, args = (count, hora, minuto,segundo, timeframe, acao, martingale,configuracao('valor_entrada'),configuracao('delay'),asset))
	thread.start()
	count += 1
